...
def _publish_date_from_status(video_id: str) -> str:
    return _row_from_status(video_id).get("原始发布日期", "").strip()
...
def write_notes_if_needed(bil: list, title: str, source: str, path: Path, url: str = "", publish_date: str = ""):
    """节目笔记：来源(含原始视频链接+原始发布日期) / 本期播客简介 / 本期嘉宾 / 时间戳 / 精彩内容 / 播客信息补充。"""
...
def main():
    ...
    publish_date = _publish_date_from_status(a.video_id)
    ...
    write_notes_if_needed(bil, title, source, job / f"{prefix}__简介与亮点.md", url=url, publish_date=publish_date)
