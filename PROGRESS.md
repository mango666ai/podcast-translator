...
## 2026-09-03 · 排查用户反馈的 4 个存量音频问题，查出 3 个互不相同的根因；修掉其中 2 个 + 还掉说话人标注技术债

用户一次反馈了 4 个问题，逐个查完发现**不是同一个病**，而是三种互相独立的故障，外加一个从来没做过的事。

### ① `>>` 被念成「大于大于」——已修

DHH 第 1 集里 `>>` 被读出来。`>>` 是 YouTube 自动字幕的说话人切换标记，`json3_to_segments.py`
**故意保留**它（`build_speaker_rules.py` 和 `build_multivoice_segments_from_json3.py` 靠它切说话人轮次），
但从来没有人在送进 TTS 之前把它剥掉——`tts_minimax.synthesize()` 是把 `text_zh` 原样发给 MiniMax 的。

在音频里实证到了：`P3KDebPTUrw` 第 30:00 处转写回来是「…这是我们做的一整套事情**就是大于大于**尽可能多地在应用里完成工作…」。

范围：8 集字幕带 `>>`（已发布 4 集：`1jlukb7gm4` 32 处、`P3KDebPTUrw` 47 处、`gSUMFvc8br0` 1 处、
`tivaWTTVRhY` 10 处；staging 4 集：`DFImJfJGXl0` 91、`NYFGCESmikA_p1` 77、`zegYJ6dhIg4` 76、`LNSvp-9b-J0` 17）。
另有 36 处 `[音乐]`/`[笑声]`/`[掌声]` 同样会被念出来。

修法：新增 `tts_minimax.clean_for_tts()`，在 `synthesize()` 入口统一剥掉，两条配音链路（单声线
`youtube_dub.py`、多声线 `youtube_multivoice_dub.py`）都从这里过，是唯一收口点；上游**不动**，
因为说话人切分还要用 `>>`。`write_srt()` 也用同一个函数，避免字幕和配音一个有一个没有。

### ② `P3KDebPTUrw`（OpenAI 合并 Codex）：翻译是对的，乱的是音频——已修根因，存量待重跑

把 432 段中文逐段和 YouTube 英文原始分段做了对齐比对（先用 `json3_to_segments.py --max-chars 260`
复现出**完全相同的 432 段、时间戳逐条 Δ=0.0**，确认比对基准没错），结论是**译文没有编号错位、也没有整段重复**。

真正的问题在音频：`youtube_multivoice_dub.py` 的分段缓存文件名是
`{stem}__{label}__{speaker}__seg_{i:04d}.mp3`——**只认序号，不认文本内容**。
2026-08-11 修完翻译截断、2026-08-20 修完编号错位之后各「替换过一次线上音频」，
但序号没变的段落全部命中旧缓存，直接复用了**旧译文合成的音频**。
所以线上 mp3 是新旧两版译文混在一起：同一句话用两种措辞前后念了两遍。

实证：音频 30:00 处念出来的「这样它才能成为最佳工具」「尽可能在应用里做更多事情」「都有一种愿望」
「这是我们做的一整套事情」这四句，在当前字幕里**一次都不存在**；字幕里是「才能变得更好」
「尽可能多地在应用里完成工作」「我们所有人都渴望」「这是我们常做的事情」。

修法：新增 `tts_cache.py`，每个 seg mp3 旁边写一个 `.txt` 指纹（voice + speed + 文本的 sha256 前 16 位），
命中缓存前先比对，对不上就重新合成。没有指纹的老缓存目录**默认判为不可信**（宁可重合成也不要再混一次），
确实要省额度可以显式 `--trust-legacy-cache`。

### ③ `tivaWTTVRhY`（Anthropic 走向垂直化）：字幕本身就重复——未修

和 ② **不是同一个病**。这集是 DeepSeek 把被 YouTube 滚动字幕切在半句的英文片段「补全」成了整段意思，
于是连续几条中文字幕在讲同一件事，音频里就连着念 4-5 遍。

实证：`#158`-`#163` 六条，每条都把「回退系统 / Opus 4 / 模型保障包」这一整段重讲一遍。
英文原文 `#158`-`#162` 是严格顺序、互不重叠的半句片段，重复完全是翻译层造出来的。

检测手段（可复用）：中文字符数 / 英文字符数的比值，正常中位数 0.29-0.31，
**比值 > 0.85 基本就是整段展开**。这集 12/384 段（约 3%）超标，集中在 `#154`-`#173`，
其中 `#168`-`#173` 是 Mercury 广告口播（比值高到 1.9-2.56）。`ByOF8qByGHU` 和 `P3KDebPTUrw` 这项都是 0%。

### ④ `ByOF8qByGHU`（Shopify Farhan Thawar）26:49 附近重复三次——未修

第三种病：DeepSeek 批量翻译的**编号错位**，而且是 2026-08-20 加了 echo 回显校验之后仍然漏网的
（这集 08-24 才发布）。`#83`-`#92` 的译文整体比原文超前 2→7 个序号（逐条核对过 EN/ZH 对照），
于是 `#88`-`#92` 提前念了 `#93`-`#97` 的内容，到 `#93` 又归位重念一遍，`#98`-`#100` 再残留一次，
一共同一段话念了三遍。

**echo 校验为什么没抓到**：`_echo_mismatch()` 校验的是 `echo` 字段和原文的对齐，
但 DeepSeek 可以把 `echo` 按正确序号抄对、同时把 `zh` 字段的内容错位——校验不到译文本身。
这是个真实的校验盲区，修法待定（候选：要求模型连原文一起回抄并做整句相似度校验；
或者放弃批内编号、改成小批+严格长度校验）。

顺带扫到 `zxvyO5vnknI`（Sam Lambert）也有 2 条重复段（`#89`、`#92`）。

### ⑤ 「how to speak 处理音量了吗」——没有

`loudnorm` 是 2026-09-02 才加进 `_concat()` 的，只对之后新拼接的音频生效。
实测 20 集已发布音频**没有一集**做过响度归一化：

| 集数 | mean_volume |
| --- | --- |
| `gSUMFvc8br0` | -18.8 dB（最响） |
| `P3KDebPTUrw` | -19.1 dB |
| `tivaWTTVRhY` | -21.4 dB |
| **`Unzc731iCUY`（how to speak）** | **-22.2 dB** |
| `ByOF8qByGHU` | -22.9 dB |
| `I8YnwUV2C9w` | -25.6 dB |
| `Z5M33oh-SAU` | -25.9 dB |
| `Yz03ERsfDDM` | -26.3 dB（最轻） |

对比修好的 `LNSvp-9b-J0` 是 -16.2 dB。staging 8 集里也只有 `LNSvp-9b-J0` 一集是 -16.2，
其余 7 集在 -19.3 ~ -25.9 之间（`DFImJfJGXl0`/`zegYJ6dhIg4`/`NYFGCESmikA_p1` 都是 09-02 加 loudnorm **之前**生成的）。

补救很便宜：纯 ffmpeg `loudnorm` 重跑一遍最终 mp3，**不烧任何 TTS 额度**。等用户拍板要不要动线上文件。

### ⑥ 还掉 `build_speaker_rules.py` 的技术债——已修

`label_turns()` 原来把全部发言轮次塞进一个 prompt，DHH 那集 1008 轮直接把 DeepSeek 输出撑到截断，
JSON 连续解析失败 3 次后 `raise` 整集崩退。改成和 `youtube_dub._translate_batch` 同一套路数：
`BATCH=50` 分批 + 失败对半拆分递归重试 + 批间传递上一批末尾 3 轮的判定结论保持对话线索；
单轮仍失败就沿用上一轮 speaker 并在结尾汇报兜底轮次，不再让整集崩掉。

冒烟验证：拿 `P3KDebPTUrw` 的 186 轮跑前 120 轮，3 批全部成功，120/120 拿到标注，
分布 main 69 / host 51，合并成 101 条规则。

### 其它

- 新增 [阿里云OSS中转指南.md](阿里云OSS中转指南.md)：CosyVoice 声音复刻卡在「需要国内可访问的音频 URL」，
  这份指南把 bucket→RAM 最小权限→签名 URL→自检→接 CosyVoice 全流程写死，等用户拍板要不要开。

### 本次验证结果

- `clean_for_tts()` 单测：`>> 我无比兴奋。`→`我无比兴奋。`，`[笑声]`/`[音乐]` 被剥掉，正常文本不动。
- `tts_cache` 单测：无文件/旧缓存无指纹/指纹一致/文本变/声音变/语速变 六种情况判定均正确。
- `build_speaker_rules` 分批标注真实跑通（见上）。
- `youtube_dub.py`、`youtube_multivoice_dub.py` import 均通过。
- ⚠️ **两条配音链路改完之后没有跑过真实合成**——MiniMax 额度仍是用尽状态，充值后第一集要盯着日志确认
  「缓存失效（旧缓存没有指纹文件），重新合成」这行是否符合预期。

### 下一步（等用户拍板的三件事）

1. **存量返修范围**：`P3KDebPTUrw` 要重合成（音频新旧混杂，最严重）；`tivaWTTVRhY` `#154`-`#173` 要重译重合成；
   `ByOF8qByGHU` `#83`-`#100` 要重译重合成；`zxvyO5vnknI` `#89`/`#92` 小修。
   8 集带 `>>` 的要不要全部重合成——这个最烧额度，需要用户决定是「全修」还是「只修听感明显的」。
2. **响度归一化**：要不要对 20 集已发布 + 7 集 staging 批量跑 loudnorm（不烧额度，但会改动线上文件）。
3. **CosyVoice**：要不要开 OSS 按指南跑通。

### 仍然阻塞

- MiniMax 额度用尽：`P06RgnUKX_I`（397/479 段）、`87DyyMV0kCY`（90/169 段）续跑，DHH 第 2-5 集配音，
  以及上面所有需要重合成的返修，全部要等额度恢复。
- `_echo_mismatch()` 的校验盲区未修，在修好之前，新处理的集数仍可能出现 ④ 那种编号错位。
...
- **bug 3：复制到写了一半的文件**。监控循环用 `[ -f 文件 ]` 判断 ffmpeg 拼接完成，但文件一创建就存在、之后还要写一分多钟，导致 p3 第一次入 staging 的是只写了 2332s 的半成品（实际 3570s）。修复：`stage_episode.py` 加 `wait_until_stable()`，连续 3 次检测文件大小不变才动它。
- **DHH 第2-5集全部完成并入 staging**，均为 CosyVoice v3.5-plus / 语速 0.92：

  | 集 | 段数 | 中文字数 | 时长 | 字/秒 |
  | --- | --- | --- | --- | --- |
  | p2 手工编程的终结 | 383 | 19396 | 63.1分 | 5.13 |
  | p3 AI编程模型与视频生成 | 401 | 18803 | 59.5分 | 5.27 |
  | p4 育儿与Linux桌面之争 | 427 | 20769 | 69.3分 | 5.00 |
  | p5 政治长寿与哲学思辨 | 302 | 14386 | 50.6分 | 4.74 |

  验证：四集的「片段时长合计 vs 成品时长」全部吻合，字/秒都在正常区间（4.7-5.3），无截断。
- 成本：四集合计约 7.3 万中文字符，按 CosyVoice v3.5-plus 的 1.5元/万字符算约 **11 元**。
- 下一步：
  1. 用户试听 staging 里的 4 集（另加之前的 Stanford 经济学课、Adam Ward 两集，共 6 集待审）
  2. 确认后按 §3.9-B 逐集转正式；⚠️ p5 内容涉及政治/哲学话题，参考 Lulu 那集的教训，转正式前留意平台审核风险
  3. 剩余：Paper/YC（TTS 卡在 397/479）、OpenAI-HuggingFace（卡在 90/169）两集需决定是充 MiniMax 续跑还是改用 CosyVoice 重做；Figma Config 54 集 + AI Engineer 26 集尚未开始
...
## 2026-08-20 · 【第二个独立bug】发现并修复 DeepSeek 批量翻译"内部编号错位"导致的语义错位，4集已发布内容全部修复；完成85集选片+builder身份分类+发布排期

- 发现过程：处理 Cursor Compile 26 开场演讲(`fWa7uxyhVDE`，3人接力演讲)新集时，用户听完已发布的"三位产品负责人"那期(`gSUMFvc8br0`)反馈第5:24、19:42附近"声线感觉不对"。排查发现根因**不是说话人分轮问题，而是翻译层一个和 2026-08-11 那次「截断→空翻译」完全不同的新 bug**：DeepSeek 批量翻译时偶尔会把批内的 `i` 编号内部搞乱——返回的编号集合完整（0..24都有）、每条译文都非空，**但具体哪个编号对应哪句英文原文的映射关系是错的**，表现为"内容完整通顺，但讲的是另一件事"（比如原文讲个人时间管理，译文却讲公司如何做营销）。这种错位 2026-08-11 那版校验（编号完整性+非空）完全测不出来，只能靠人工听感或语义比对发现。
- 代码修复：`youtube_dub.py` 的 `_translate_batch()` 提示词新增 `echo` 自查字段——要求 DeepSeek 对每条译文额外原样抄写对应原文开头5-8个英文单词；新增 `_echo_mismatch()` 用词汇重叠率（阈值0.3）比对 `echo` 与原文开头，重叠过低即判定为编号错位，触发重试（沿用已有的3次重试+连续失败对半拆分递归逻辑）。
- 已发布集数抽查（用户要求"先抽查一下"，暂停85集批量处理前先核查历史集数）：并行起5个agent逐条比对5集（`Unzc731iCUY`/`P3KDebPTUrw`/`1_jlukb7gm4`/`tivaWTTVRhY`/`fWa7uxyhVDE`）双语分段的英文原文与中文译文语义是否对应，结果：
  - `Unzc731iCUY`（How to Speak）：干净，未发现问题
  - `P3KDebPTUrw`（第9集）：基本干净，仅 index 191/192 译文完全重复（192自己的原文从未被单独翻译），非编号错位但顺手一并修了
  - `fWa7uxyhVDE`（Cursor开场演讲）：index 20-24（批次末尾）错位
  - `tivaWTTVRhY`（Dianne Penn）：index 52-74（尤其64-74十一条共享同一段译文）+ index 153-155（155是"（译文省略）"空占位符）错位，是目前发现最严重的一例
  - `1_jlukb7gm4`（Alex Lieberman）：index 20-24 + index 103-111（访谈正文与Customer.io广告插播内容被打乱穿插）错位
- 修复流程（4集全部走完）：用新的 echo 校验版 `_translate_batch()` 重译对应完整批次（按25段批次边界，不只重译报错的几条）→ 删除对应 index 的 `seg_full/` 缓存 mp3 → 用 `youtube_multivoice_dub.py` 复用未受影响片段+只重新合成变更片段 → 替换 `episodes/` 下的发布音频 → 更新 `feed.xml` 对应 item 的 `itunes:duration`/`enclosure length` → 更新 `podcast_status.csv` 备注 → 每集单独 commit+push（避免 2026-08-11 那次"一次 push 带出下一集未到发布时间的内容"的操作失误重演）。
- 选片+排期（用户提供3个大会/频道候选清单后完成）：
  - 对全部265个候选讲者做了"builder身份"分类（是否是产品/作品的实际构建者本人——排除纯投资人、行业分析师、无产品归属的学者），逐一附中文身份说明，见 [待处理视频_Builder身份分类.md](待处理视频_Builder身份分类.md)
  - 用户选定范围：Cursor Compile 5集"是" + Figma Config 54集"是"（排除用户明确要求跳过的8条"产品向deep dive"）+ AI Engineer频道用户圈出的约26条"有分量"视频，共85集
  - 按"每天2-3集、单日总时长≤65分钟、单集≥60分钟单独占一天、发布时段限定8:30/9:00/12:00/18:00"排出37天排期，见 [待处理视频_发布排期.md](待处理视频_发布排期.md)
  - 新增 staging RSS（`staging/feed.xml`，标题"Dove的AI译站（待审核）"）作为试听专用中转，用户后续会在小宇宙自建一个栏目订阅它；首集(`fWa7uxyhVDE`)已用户确认后直接转发布到正式 feed
- 环境修复：`yt-dlp` 版本过旧导致 YouTube "n challenge" 触发 403，`uv tool upgrade yt-dlp` 解决，对后续84集下载同样适用
- 下一步：
  1. 4集修复内容已全部 push，等 GitHub Pages/客户端缓存刷新后订阅者会拿到修复版
  2. 按 [待处理视频_发布排期.md](待处理视频_发布排期.md) 继续推进剩余84集（Day2起为 Cursor Compile 剩余4集：`ByOF8qByGHU`/`I8YnwUV2C9w`/`zxvyO5vnknI`/`Z5M33oh-SAU`）
  3. 是否要把 echo 校验回溯应用到其余未抽查的历史集数（当前只抽查了这5集），待观察是否有进一步反馈决定
